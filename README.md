...
| Strip slicer feature/metadata comments | `;TYPE:`, `;WIDTH:`, wipe/flush/object markers, Orca's HEADER/THUMBNAIL/EXECUTABLE block delimiters, `; generated by …`, `; filament_*:`, `; * extrusion width = *`, Klipper `;_SET_FAN_SPEED` hints, bare `;` separators. Preserves markers BTT TFT firmware and the Mintion Beagle web UI read: `LAYER_CHANGE` / `LAYER_COUNT` (layer counter), and `;HEIGHT:` / `;Z:` / `; total layer number:` / `; max_z_height:` (height + current-layer tiles in the Beagle dashboard). |
...
| `ENABLE_STRIP_FEATURE_COMMENTS` | `True` | Drop `;TYPE:`, `;WIDTH:`, wipe markers, generated-by line, filament info, extrusion-width annotations, Klipper `;_SET_FAN_SPEED` hints, bare `;` separators. `LAYER_CHANGE` / `LAYER_COUNT` and the height markers (`;HEIGHT:`, `;Z:`, `; total layer number:`, `; max_z_height:`) are preserved because the Beagle web UI and BTT TFT firmware read them. | Lose human-readable annotations if you ever open the file. |
